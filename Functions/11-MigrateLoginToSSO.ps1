...
$dataToProcess = $dataToProcess | Where-Object {$_."Status" -eq "CLEAN"}
...
foreach($row in $dataToProcess){
    ...
    $userObj.dataToUpdate.nickname = $row.email
...
foreach($group in $groups){
    ...
    $resp = $connector.SendRequest([OARequestType]::ModifyBulk,$params)
